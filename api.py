...
from main import run_pipeline, run_plan

load_dotenv()

# Broken local proxy settings often break ZenMux/OpenRouter HTTPS calls.
for _proxy_var in (
    "HTTP_PROXY",
    "HTTPS_PROXY",
    "ALL_PROXY",
...
def _run_task(record: TaskRecord, request: GenerateRequest) -> None:
    ...
    except Exception as exc:  # noqa: BLE001 - surface pipeline failures to the task record
        record.status = "error"
